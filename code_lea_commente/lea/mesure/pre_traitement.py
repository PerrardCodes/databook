...
def charge_video_ram(fichier, Dic, nb_im=20):
	"""
	Cette fonction charge une vidéo dans la ram

	:param fichier: Vidéo
	:type fichier: Adresse de la vidéo
	:param Dic: Dictionnaire devant contenir toutes les informations
	:type Dic: dict
	:param nb_im: Nombre d'image de la vidéà à charger dans la ram
	:type nb_im: int
	"""
	video = fichier
...
def charge_image_ram(fichier, Dic, extension, nb_im=20):
	"""
	Cette fonction charge une ou plusieurs images dans la ram

	:param fichier: Une ou plusieurs images
	:type fichier: Adresse de l'image
	:param Dic: Dictionnaire devant contenir toutes les informations
	:type Dic: dict
	:param nb_im: Nombre d'image du dossier à charger dans la ram
	:type nb_im: int
	"""
...
def get_im_video(video, num):
	"""
	:return: l'image numéro num dans une vidéo
	"""
	vidcap = cv2.VideoCapture(video)
	success, image = vidcap.read()
	count = 0
...
def get_im_files(fichier, num, extension) :
	"""
	:returns: Image numéro num dans un fichier
	"""
	if os.path.isdir(fichier):
		files = glob.glob(fichier + "/*." + extension)
		files = tri_insertion(files)
...
def get_data_param(Dic, mesure, fx, fps, D):
	"""
	Récupère le fx, ft, D depuis Data s'ils existent sinon il prend ceux passé en paramètres.
	Le D doit être en mm, il est donc convertit si nécessaire
	"""
...
def get_H_and_L(Dic, mesure):
	"""
	Récupère la hauteur et la largeur soit de l'image soit de la vidéo.
	"""
...
def set_axes(Dic, x0, y0, xmin, xmax, ymin, ymax):
	"""
	Pour crop l'image il faut en premier vérifier que les x,ymin, x,ymax, x,y0 soient corrects
	"""
...
def crop(Dic, im):
	"""
	Crop une image avec xymin, xymax
	"""
...
def image_to_array(Dic, im):
	"""
	Passe une image PIL en np array
	"""
	pix = im.load()
...
def in_dict(Dic, thetamin, thetamax, rmax, rmin, B):
	"""
	Met dans le dictionnaire les dernières informations nécessaires
	"""
...
def redefine_rmax(Dic, DF):
	"""
	Pour la classe :mod:`lea.mesure.Contour` on a besoin de redéfinir le rmax à chaque tour pour suivre le ballon
	"""
...
def redefine_center(Dic, DF):
	"""
	Pour la classe :mod:`lea.mesure.Contour` on a besoin de redéfinir le centre à chaque tour pour suivre le ballon
	"""
	r = []
	std = []
