...
class PIVDataProcessing:
 ...
 #       print(len(self.Vlist))
    ...
    def run_analysis(self,a_frames=None,a_volumes=None,zaxis=0,directions=[1,2],save=True,s2n_thresh=1.3,bg_n_frames=None):
        ...
        Vol = self.Vlist[0]
        v = Vol.m['volume']        
        self.volume_shape = v.shape
        
        self.cine_frame_shape = self.volume_shape[directions]
        length = self.volume_shape[zaxis]
        
        print(self.cine_frame_shape)        

        if a_frames is None:
            a_frames = np.arange(0,len(self.Vlist)-frame_diff)
        if a_volumes is None:
            a_volumes = np.arange(0,length)

                #np.arange(0,len(Vlist)-frame_diff)

#        if bg_n_frames is not None:
#            bg = backlight.construct_bg_image(Vlist,n_frames=bg_n_frames,usemax=False)
#           plt.figure()
#            plt.imshow(bg)
...
#                return im
        ...
        shape = np.shape(crop(v))
        
        flow_field = init_flowfield(len(a_frames),shape,self.window_size,self.overlap)
        ...
        self.dt_ab = self.dt_orig*self.frame_diff # dt between frames a and b
        self.dt_frames = self.dt_orig * np.median(np.diff(self.a_volumes)) # ASSUMES THAT ALL A FRAMES ARE EVENLY SPACED! -> dangerous
        try:
            self.a_frame_times = [float(self.Vlist[i].m['tV']) for i in a_volumes] # time of the a frames
        except:
            print('frame time stamps not available')
            self.a_frame_times = a_frames*self.dt_frames

        for vii,vi in enumerate(a_volumes):
            volume_a = self.Vlist[vii].m['volume']
            volume_b = self.Vlist[vii+self.frame_diff].m['volume']
...
#            u,v = process_frame(self,frame_a,frame_b,s2n_thresh=1.3):
            ...
            # store the velocity fields
                ...
                if (save==True)&(ai%100==0): # save every some frames, just in case
                    filename = self.parent_folder+self.name_for_save+'_flowfield.npy'
                    np.save(filename,flow_field)
                    self.flow_field_res_filepath = filename
                    self.save()
                    print(filename)

        if save==True:
            np.save(self.parent_folder+self.name_for_save+'MP_flowfield.npy',flow_field)
...
if __name__ == '__main__':
    ...
    date = '20181010'
    save_folder = 'DATA/Experimental_data/Turbulence3d/'+date+'/processed'

    # dx and dt for the cine file
    #dx = 3.3240951317E-05 # viewA
    #dx = 8.4191870904E-05 # viewB
    dx =  0.3457E-03#7.3469357156E-05
    dt_orig = 1./40000

    crop_lims=None
    pre_constructed_masker = None
    a_frames = np.arange(0,119960,1)
    frame_diff = 40
...
#    os.mkdir(parent_folder+save_folder)
    ...
    '''
    processing = PIVDataProcessing(parent_folder,cine_name,name_for_save=save_folder,dx=dx,dt_orig=dt_orig,frame_diff=frame_diff,crop_lims=crop_lims,maskers=None,window_size=window_size,overlap=overlap,search_area_size=32)
...
# Load existing flowfield
    ...
    dz = 2.15
    z = np.arange(-Nz/4,Nz/4)*dz
    x = np.arange((Nx-1)/2,-(Nx-1)/2-1,-1)*processing.dx*1E3*overlap+205
    y = np.arange(-(Ny-1)/2,(Ny-1)/2+1)*processing.dx*1E3*overlap
    ...
    if d==3:
        ...
        cmaps = ['PuOr','seismic']
        count=0
        fig,axs=plt.subplots(2,1,figsize=(16,6)); axs=axs.flatten()
        for j in range(Nx-2,0,-1):

            c = []
            for (i,data,v) in zip([0,1],[mean_flow[:20,j,:,1],u_rms[:20,j,:]],[1.2,0.4]):
                divider = make_axes_locatable(axs[i])
                cax = divider.append_axes('right', size='10%', pad=0.1)

                cb=axs[i].pcolormesh(Y,Z,data,vmin=0,vmax=v)
...
#                graphes.legende('x (mm)','y (mm)','Mean flow')
            ...
            c[1].set_label('uz_rms'' (m/s)')

            axs[1].set_xlabel('x (mm)')
            axs[0].set_title('Mean flow, z = '+str(x[j])+' mm')
            axs[1].set_title('Fluctuations')

            #plt.show()
            plt.pause(0.1)

            filename = '/media/stephane/DATA/Experimental_data/Turbulence3d/20181010/scan_3d/'+str(count)
