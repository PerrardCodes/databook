...
def h5py_in_Mesure(f):
	...
	if(f.__contains__("Bulles")) :
		...
		mesure.add_measurement(b)
	if(f.__contains__("PIV3D")) :
		p = h5py_in_piv3d(f, data)
		mesure.add_measurement(p)
	return mesure
...
def creation_utilisateur():
	...
	spec = {}
	data = Data(fichier, param, spec, index=index, typ=typ, who=who, date=date, heure=heure)
	return data
